...
#ifdef DIGGER_ELKS
#define DIGGER_DIRBUF_TIMEOUT 6
#define DIGGER_DIRBUF_NONE    255

static unsigned char buffered_dir[DIGGERS];
static unsigned char buffered_age[DIGGERS];

static bool dirbuf_is_dir(Sint4 dir)
{
  return (dir==DIR_RIGHT || dir==DIR_UP || dir==DIR_LEFT || dir==DIR_DOWN);
}

static void clear_dirbuf(int n)
{
  if (n>=0 && n<DIGGERS) {
    buffered_dir[n]=DIGGER_DIRBUF_NONE;
    buffered_age[n]=0;
  }
}

static bool dirbuf_active(int n)
{
  return (n>=0 && n<DIGGERS && buffered_dir[n]!=DIGGER_DIRBUF_NONE);
}

static bool dirbuf_turn_legal(int dig, unsigned char dir)
{
  if ((dir==DIR_UP || dir==DIR_DOWN) && digdat[dig].rx==0)
    return TRUE;
  if ((dir==DIR_RIGHT || dir==DIR_LEFT) && digdat[dig].ry==0)
    return TRUE;
  return FALSE;
}

static void age_dirbuf(int n)
{
  if (!dirbuf_active(n))
    return;
  buffered_age[n]++;
  if (buffered_age[n]>=DIGGER_DIRBUF_TIMEOUT)
    clear_dirbuf(n);
}
#endif
...
unsigned int elks_newframe_count;
unsigned int elks_live_frame_count;
unsigned int elks_pace_wait_count;
unsigned int elks_pace_sleep_count;
unsigned int elks_pace_wait0_count;
unsigned int elks_pace_late_count;
unsigned int elks_pace_late2_count;
unsigned int elks_pace_back_count;
unsigned int elks_pace_sleep_max;
unsigned int elks_pace_late_streak_max;
static unsigned int elks_pace_late_streak;
#endif
...
#ifdef DIGGER_ELKS
    t=gethrt();
    target=curtime+ftime;

    /*
     * ELKS gettimeofday() is millisecond-granularity here.  On fast QEMU
     * builds several game frames can arrive before gethrt() advances past
     * curtime.  Treat t<=curtime as still early, not as a reason to skip
     * pacing.  Keep the old slow-machine behavior: only avoid sleeping when
     * wall time has really passed the next scheduled frame.
     */
    if (t<target) {
      ...
      if (waitms>0) {
        elks_pace_sleep_count++;
        if ((unsigned int)waitms > elks_pace_sleep_max)
          elks_pace_sleep_max = (unsigned int)waitms;
      }
      else
        elks_pace_wait0_count++;
#endif
      fillbuffer();             /* No-op in the ELKS silent build. */
      checkkeyb();              /* Sample once before sleeping. */
      if (waitms>0)
        olddelay(waitms);
      curtime=target;           /* Preserve cadence across coarse ticks. */
    }
    else {
#ifdef DIGGER_CGA_PROFILE
      elks_pace_late_count++;
      elks_pace_late_streak++;
      if (elks_pace_late_streak > elks_pace_late_streak_max)
        elks_pace_late_streak_max = elks_pace_late_streak;
      if (t-target >= ftime)
        elks_pace_late2_count++;
#endif
      curtime=t;                /* Already late: avoid sleeping on slow 8086. */
    }
    checkkeyb();
#else
    do {
...
#ifdef DIGGER_ELKS
#ifdef DIGGER_ELKS_DOS_SOUND
  /* The original DOS sound state machine is advanced by the DOS timer
   * path at PIT/0x4000 ~= 72.8 Hz, independent of the game frame rate.
   * ELKS has no restored timer interrupt backend, so advance the state
   * machine by the equivalent number of logical sound ticks per frame.
   * For the normal ELKS ftime=80000 this gives a 4/5 tick cadence.
   *
   * Keep the Stage 18d immediate speaker writes: the PC speaker is touched
   * on each logical DOS sound tick, not smoothed to the final tick of the
   * video frame.  This sounds raspier and closer to the DOS backend.
   */
  {
    static Uint5 elks_sound_accum;
    elks_sound_accum += ftime;
    while (elks_sound_accum >= 0x4000ul) {
      soundint();
      elks_sound_accum -= 0x4000ul;
    }
  }
#else
  /* The small direct backend is frame-tick based by design. */
  soundint();
#endif
#endif
...
void updatedigger(int n)
{
...
#ifdef DIGGER_ELKS
  /* ELKS can have long live frames when several enemies are active.  Keep a
   * tiny directional intent buffer so a turn pressed a few frames early is not
   * lost before Digger reaches the grid alignment where the original movement
   * code can accept it.  Only directions are buffered; fire/menu/pause/escape
   * still follow the normal keyboard path.
   */
  if (dirbuf_is_dir(dir)) {
    if (dirbuf_active(dn)) {
      if ((unsigned char)dir==buffered_dir[dn])
        buffered_age[dn]=0;
      else
        if (dir!=digdat[n].dir) {
          buffered_dir[dn]=(unsigned char)dir;
          buffered_age[dn]=0;
        }
        else
          age_dirbuf(dn);
  ...
  ddir=DIR_NONE;
  if (dirbuf_active(dn) && dirbuf_turn_legal(n,buffered_dir[dn])) {
    ddir=(Sint4)buffered_dir[dn];
    clear_dirbuf(dn);
  }
  else
    if (dirbuf_is_dir(dir))
  ...
  dir_input=dirbuf_is_dir(dir) || ddir!=DIR_NONE || dirbuf_active(dn);
#else
...
#ifdef DIGGER_ELKS
    eatfieldgrid(nh,nrx,nv,nry,digdat[n].mdir);
#else
  ...
  if (hitemerald(nh,nv,nrx,nry,digdat[n].mdir)) {
    if (digdat[n].emocttime==0)
      digdat[n].emn=0;
    scoreemerald(n);
    soundem();
...
#ifdef DIGGER_ELKS
        if (digdat[n].h==h)
          if (digdat[n].v==v || digdat[n].v+1==v)
            return TRUE;
#else
