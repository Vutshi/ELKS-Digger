...
void getinitials(void)
{
  ...
  killsound();
#ifdef DIGGER_ELKS
  /* Discard the last gameplay/start key before blocking for initials.
   * ELKS synthetic arrow keys are values such as 0x14d; if one reaches this
   * code path it draws as blank but stores low byte 0x4d ('M') in the score
   * table.
   */
  elks_input_flush_pending();
  elks_input_set_text_mode(TRUE);
#endif
  for (i=0;i<3;i++) {
    k=0;
    while (k==0) {
    ...
    if (k!=0) {
      ...
      scoreinit[0][i]=(char)k;
    }
  }
#ifdef DIGGER_ELKS
  elks_input_set_text_mode(FALSE);
  elks_input_flush_pending();
#endif
  for (i=0;i<20;i++)
#ifdef _WINDOWS
