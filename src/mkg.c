...
static void write_ascii_table(const char *prefix)
{
  int i,x,b,p,l;

  fprintf(out,"Uint3 near *ascii2%s[0x5f]={\n",prefix);
  l=0;
  p=32;
  for (i=32;p<127;i++) {
    for (x=0;clist[x]!=0;x++)
      if (toupper(clist[x])==toupper(i)) {
        l+=strlen(symbols[clist[x]-32])+strlen(prefix)+1;
        break;
      }
    if (clist[x]==0)
      l+=3;
      ...
      for (b=p;b<i;b++) {
        for (x=0;clist[x]!=0;x++)
          if (toupper(clist[x])==toupper(b)) {
            fprintf(out,"%s%s",prefix,symbols[clist[x]-32]);
            l+=strlen(symbols[clist[x]-32])+strlen(prefix)+1;
            if (b<126)
              fprintf(out,",");
            break;
...
int main(int argc,char *argv[])
{
  ...
  if (argc<=1) {
    fprintf(stderr,
            "Syntax: MKG <name>\n"
            "Where <name> is vgagrafx.c, cgagrafx.c, alpha.c, or alpha_cga.c\n");
    return 1;
  }

  target=basename_c(argv[1]);

  if (stricmp(target,"vgagrafx.c")==0)
    n=0;
  if (stricmp(target,"cgagrafx.c")==0)
    n=1;
  if (stricmp(target,"alpha.c")==0)
    n=2;
  if (stricmp(target,"alpha_cga.c")==0)
    n=3;
