...
void erasetext(Sint4 n,Sint4 x,Sint4 y,Sint4 c)
{
  while (n-- > 0) {
    gwrite(x,y,' ',c);
    x+=12;
...
void drawstatics(void)
{
  ...
  drawbackg(levplan());
#ifdef DIGGER_ELKS
  /* During the initial level static draw there are no enabled runtime sprites
   * yet.  Avoid the misc-sprite save/restore/overlap bookkeeping used by
   * runtime tunnel carving; drawfield() only deposits dirt/blob masks here.
   */
  static_field_draw=TRUE;
#endif
  drawfield();
#ifdef DIGGER_ELKS
  static_field_draw=FALSE;
#endif
}
...
#ifdef DIGGER_ELKS
#define FIELDIDX(h,v) (((v)<<4)-(v)+(h))
/* Convert cached pixel y remainder 0,3,6,9,12,15 to DOS eatfield row. */
#define FIELDYBIT(yr) ((yr)==15 ? 5 : (yr)==12 ? 4 : (yr)==9 ? 3 : \
                       (yr)==6 ? 2 : (yr)==3 ? 1 : 0)

void eatfieldgrid(Sint4 h,Sint4 xr,Sint4 v,Sint4 yr,Sint4 dir)
{
  Sint4 cell;
  incpenalty();
  switch (dir) {
    ...
    case DIR_DOWN:
      v++;
      cell=FIELDIDX(h,v);
      field[cell]&=bitmasks[6+FIELDYBIT(yr)];
      if (field[cell]&0xfc0)
        break;
      field[cell]&=0xdfff;
  }
